...
- 멀티 쓰레드 기반의 서버구현
    ...
    - 임계영역(Critical Section) : 둘 이상의 쓰레드가 동시에 실행하면 문제를 일으키는 문장이 하나 이상 존재하는 함수를 말함
        ...
        struct hostent *gethostbuname_r(const char *name, 
        struct hostent *result, char *buffer, intbuflen, int *h_errnop); // 쓰레드에 안전
        ```
    ...
    - 쓰레드 동기화
        - 동기화가 필요한 상황
            ...
            2. 동일한 메모리 영역에 접근하는 쓰레드의 실행순서를 지정해야하는 상황

        - 동기화 기법 : 동기화를 통해서 동시 접근을 막고, 접근의 순서를 지정가능
            1. 뮤텍스(Mutex) 기반 동기화
